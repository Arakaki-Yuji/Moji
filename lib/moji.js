...
Moji.prototype.camelize = function(string, format){
    ...
    if(format === 'upper'){
        resultStr = String(string).replace(/^[a-z]|_[a-z]/g, function(match){
            match = match.replace(/_/, '');
            return match.toUpperCase();
        })
    } else if (format === 'lower'){
        resultStr = String(string).replace(/_[a-z]/g, function(match){
            match = match.replace(/_/, '');
            return match.toUpperCase();
        })
    } else {
...
Moji.prototype.underscore = function(string){
    resultStr = String(string).replace(/^[A-Z]/g, function(match){
        return match.toLowerCase();
    });

    resultStr = String(resultStr).replace(/[A-Z]/g, function(match){
...
Moji.prototype.escapeHtml = function(string){
    ...
    return String(string).replace(/<br>|<\/br>|<br \/>|[&"<>]/g, function(match){
        // 改行タグは許可する
